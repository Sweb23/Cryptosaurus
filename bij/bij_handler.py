...
def bij_display(toplevel):
    ...
    Button(toplevel, image=im8, command=None).grid(column=0,row=4)
    
    
    im9 = create_img("bij/img/pur.png")
    Button(toplevel, image=im9, command=None).grid(column=2,row=4)
...
def toplevelForm(form):
    ...
    form.createForm()
    Button(form_window, text="Close",command=form_window.destroy).grid(column=1,row=11)
